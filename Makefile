...
bootstrap-tools: $(TEMP_DIR)
	...
	curl -sSfL https://raw.githubusercontent.com/khulnasoft/go-licenses/refs/heads/master/golicenses.sh | sh -s -- -b $(TEMP_DIR)/ $(GOLICENSES_VERSION)
...
generate-test-data:
	docker build -t inspo-test:latest -f .data/Dockerfile.test-image . && docker image save -o .data/test-docker-image.tar inspo-test:latest && echo 'Exported test data!'
...
ci-test-docker-image:
	docker run \
		...
		'${PRODUCTION_REGISTRY}/khulnasoft/inspo:latest' \
			'${TEST_IMAGE}' \
			--ci
	...
	docker run \
		...
		ubuntu:latest \
			/bin/bash -x -c "\
				...
				docker version && \
				apt install ./snapshot/inspo_*_linux_amd64.deb -y && \
				inspo --version && \
				inspo '${TEST_IMAGE}' --ci \
			"
		...
		fedora:latest \
			/bin/bash -x -c "\
				curl -L 'https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_CLI_VERSION}.tgz' | \
					...
					mv docker /usr/local/bin/ &&\
				docker version && \
				dnf install ./snapshot/inspo_*_linux_amd64.rpm -y && \
				inspo --version && \
				inspo '${TEST_IMAGE}' --ci \
			"
...
ci-test-linux-run:
	ls -la $(SNAPSHOT_DIR)
	ls -la $(SNAPSHOT_DIR)/inspo_linux_amd64_v1
	chmod 755 $(SNAPSHOT_DIR)/inspo_linux_amd64_v1/inspo && \
	$(SNAPSHOT_DIR)/inspo_linux_amd64_v1/inspo '${TEST_IMAGE}'  --ci && \
    $(SNAPSHOT_DIR)/inspo_linux_amd64_v1/inspo --source docker-archive .data/test-kaniko-image.tar  --ci --ci-config .data/.inspo-ci
...
ci-test-mac-run:
	chmod 755 $(SNAPSHOT_DIR)/inspo_darwin_amd64_v1/inspo && \
	$(SNAPSHOT_DIR)/inspo_darwin_amd64_v1/inspo --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.inspo-ci

# we're not attempting to test docker, just our ability to run on these systems. This avoids setting up docker in CI.
.PHONY: ci-test-windows-run
ci-test-windows-run:
	inspo.exe --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.inspo-ci
